...
<x-modal
    modal_id="listing-process-view-modal-{{$car->id}}"
    title="Uploaded Vehicle Information"
>
    <x-uploadedVehicleInfo :car="$car" />
</x-modal>
<x-modal
    modal_id="listing-images-view-modal-{{$car->id}}"
    title="Uploaded Vehicle Images"
>
    <x-uploadedImagesSlider :images="$car->images" />
</x-modal>

<div
    class="w-80 min-h-90 gradient2 p-2 rounded-lg space-y-4 pb-8 transition-all hover:scale-105"
>
    ...
    <div class="flex justify-between px-2 items-center">
        ...
        <h1 class="text-xs font-bold text-orange">
            {{$car->created_at->format('D, M j, Y, g:ia')}}
        </h1>
    </div>
</div>
