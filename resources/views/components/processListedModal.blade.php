...
<div class="space-y-8">
    <form
        id="decline_listing-{{$car->id}}"
        method="post"
        action="{{route('decline_listing', $car->id)}}"
    >
        @csrf
    </form>
    ...
    <div class="flex space-x-4">
        <button
            form="approve_listing-{{$car->id}}"
            type="submit"
            class="md:w-1/2 bg-green-600 p-2 text-white"
        >
            Process
        </button>
        <button
            type="submit"
            form="decline_listing-{{$car->id}}"
            class="md:w-1/2 bg-red-600 font-semibold p-2 text-white"
        >
            Decline Listing
        </button>
    </div>
